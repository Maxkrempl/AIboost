#!/usr/bin/env python3
"""
Scrape contact info for MenuBoost prospect list.
Outputs CSV with columns: name,email,city,country,website,owner_or_chef,specialty,source
"""
...
# Known email corrections (if automatic extraction fails)
EMAIL_CORRECTIONS = {
    'hotelmarina.si': 'recepcija@hotelmarina.si',
    'kogo.si': 'info@kogo.si',
    'monte.hr': 'restaurant@monte.hr',
    'pelegrini.hr': 'reservations@pelegrini.hr',
    'valamar.com': 'reservations@valamar.com',
    'agliamici.it': 'lab@agliamici.it',
}

def extract_emails(text):
    """Extract email addresses from text, with word boundaries."""
...
def clean_email(raw, domain):
    """Extract clean email from raw string."""
    if not raw:
        return ''
    # Try to find core email by locating @ and expanding
    at_pos = raw.find('@')
    if at_pos == -1:
        return ''
    left = ''
    ...
    matches = re.findall(r'[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}', raw)
    if matches:
        return matches[0]
    return raw
...
def find_page_link(soup, base_url, keywords):
    """Find first link containing any of the keywords."""
...
def extract_owner_or_chef(soup):
    """Heuristic extraction of owner/chef name."""
...
def scrape_restaurant(name, website, city, country):
    ...
    # If still no email, try known corrections
    if not best_email and domain in EMAIL_CORRECTIONS:
        best_email = EMAIL_CORRECTIONS[domain]
    
    # Clean the email
    best_email = clean_email(best_email, domain)
...
def main():
    restaurants = [
        ...
        ("Restaurant Spinnaker", "valamar.com", "Poreč", "Croatia"),
        # Croatia (Dalmatia)
    ...
    # Write to CSV
    csv_path = 'menuBoost_premium_leads.csv'
