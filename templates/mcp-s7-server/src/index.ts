...
server.tool(
  "read_sensor",
  "Read a sensor value from the Siemens S7-1500 PLC. Returns current value with unit.",
  {
...
server.tool(
  "get_alarms",
  "Get all active alarms from the Siemens S7-1500 PLC. Returns list with priority and recommended action.",
  {},
  async () => {
    try {
      const state = await plc.getState();
...
server.tool(
  "get_status",
  "Get overall Siemens S7-1500 PLC status including all sensors, alarms, setpoints, and outputs.",
  {},
  async () => {
    try {
      const state = await plc.getState();
...
server.tool(
  "write_setpoint",
  "Write a setpoint value to the Siemens S7-1500 PLC. Use with caution! Values are validated against safety limits.",
  {
  ...
  async ({ parameter, value }) => {
    try {
      ...
      const limits: Record<string, { min: number; max: number; db: string; tag: string }> = {
...
server.tool(
  "get_all",
  "Get all Siemens S7-1500 PLC data at once: sensors, alarms, status, setpoints, outputs.",
  {},
  async () => {
    try {
      const state = await plc.getState();
...
server.tool(
  "read_db",
  "Read a Siemens S7 Data Block. Returns raw register values.",
  {
  ...
  async ({ dbNumber, startByte, byteCount }) => {
    try {
      ...
      const conn = (plc as any).conn;
      const vars = (plc as any).variables;
      vars[tag] = address;
      conn.setTranslationCB((t: string) => vars[t]);
      conn.addItems([tag]);

      const values = await new Promise<Record<string, any>>((resolve, reject) => {
        conn.readAllItems((bad: boolean, vals: Record<string, any>) => {
